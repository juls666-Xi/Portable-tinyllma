...
#!/usr/bin/env bash
# ================================================================
# TinyLlama 1.1B Q2_K — Linux Launcher
# ================================================================

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
CONFIG="$HOME/.local/share/tinyllama/config.sh"
...
echo ""
echo -e "${C}================================================${N}"
echo -e "${C} TinyLlama 1.1B Q2_K — Linux Launcher ${N}"
echo -e "${C}================================================${N}"
echo ""
...
if [ ! -f "$TINYLLAMA_MODEL" ]; then
    ...
    FOUND=$(ls "$HOME/.local/share/tinyllama/models/"*.gguf 2>/dev/null | head -1)
...
echo -e " Engine : ${W}llama.cpp (native)${N}"
...
if curl -s "http://127.0.0.1:${TINYLLAMA_LLAMA_PORT}/v1/models" >/dev/null 2>&1; then
    ...
    "$TINYLLAMA_BIN" \
        ...
        --host 127.0.0.1 \
        > "$HOME/.local/share/tinyllama/logs/llama-server.log" 2>&1 &
    ...
    until curl -s "http://127.0.0.1:${TINYLLAMA_LLAMA_PORT}/v1/models" >/dev/null 2>&1; do
        ...
        if [ "$WAIT" -ge 120 ]; then
            echo ""
            fail "Engine failed to start after 120s. Check: cat ~/.local/share/tinyllama/logs/llama-server.log"
        fi
    done
    echo ""
